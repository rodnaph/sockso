...
public class HSQLDatabase extends JDBCDatabase {
    ...
    public void connect( final OptionSet options ) throws DatabaseConnectionException {
        ...
        try {
            ...
            log.info("Connecting to database: "+ connectionString);
        ...
        checkIndexerTableUpgrade();
        checkPlaylistTableUpgrade();
    }
   ...
   private void checkMultipleSlashesInCollection() {
    ...
    /**
     *  Checks if playlists table supports playlist indexer
     *
     */

    private void checkPlaylistTableUpgrade() {

        final String sql = " alter table playlists " +
    ...
    /**
     *  Checks if index table supports multiple indexers
     *
     */

    private void checkIndexerTableUpgrade() {

        try {
            
            String sql = " alter table indexer " +
                           " add fid integer default 0 not null ";

            update( sql );

            sql = " alter table indexer " +
                           " add type varchar(300) " +
                           " default 'com.pugh.sockso.music.indexing.TrackIndexer' not null ";
            update( sql );
            
            sql = " select max(id) as val from indexer ";
            final ResultSet rs = query( sql );
                     
            int max = 0;
            if ( rs.next() && rs.getInt("val")>0) {
                    max = rs.getInt("val") + 1;
            }
            sql = " alter table indexer " +
                           " alter id integer generated by default as identity (start with " + 
                            max + ") not null ";
            update( sql );
            
            sql = " update indexer set fid = id ";
            update( sql );

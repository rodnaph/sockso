...
{include file="include/header.tpl" title="Old News"}
...
-
Bug fixes for MySQL, reduced CPU usage with {link url={ controller="manual" action="properties" } text="toggleable buffering options"} when streaming,
...
-
{link url={ controller="manual" action="mysql" } text="MySQL"} and (some) {link url={ controller="manual" action="sqlite" } text="SQLite"} support added,
Sockso is now {link url={ controller="manual" action="customize" } text="more easily customizable"}, new WMA to MP3 encoding option.
</p>
...
{link url={ controller="manual" action="scrobbling" } text="scrobble log"} for tracks
...
new {link url={ controller="manual" action="encoders" } test="ogg-mp3 encoding" } option, custom encoding ability enabled,
users can {link url={ controller="manual" action="cmdline" } text="specify data directory" } and change amount of
{link url={ controller="manual" action="runningOnServer" } text="info on www"} (eg. popular tracks)</p>
...
-
{link url={ controller="manual" action="imageflow" } text="Imageflow"} album art,
{link url={ controller="manual" action="folderBrowsing" } text="folder browsing"},
...
Released (<b>Highlights:</b> Now {link url={ controller="manual" action="customize" } text="customizable"},
...
<h3>13th July</h3>
<p>Created a {link url={ controller="source" } text="Bazaar repository"} online
...
{link url={ controller="manual" action="embedding" } text="embedding music"} in your own website!)</p>

<h3>28th June</h3>
<p>{link url={ controller="manual" } text="Sockso manual"}
